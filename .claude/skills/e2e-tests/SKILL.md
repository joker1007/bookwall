...
UI / API レベルの動作検証は **dev 環境ではなく e2e 用の test 環境** で実施する。
...
| プロセス | ポート | 役割 |
|---|---|---|
| Rails (Falcon) | 3001 | `RAILS_ENV=test` + `BOOKWALL_E2E_RESET=1` 起動 |
...
UI を立ち上げる前にロジックだけ確認したい場合は `rails runner` で `RAILS_ENV=test` を指定する:

```bash
cd server
RAILS_ENV=test BOOKWALL_E2E_RESET=1 bundle exec rails runner '
...
または `RAILS_ENV=test bundle exec rails db:drop db:prepare` で完全リセット。
...
- `webServer: reuseExistingServer: !CI` なので、開発中に手動で `RAILS_ENV=test bin/rails s -p 3001` を起動しておくと playwright がそれを使う
