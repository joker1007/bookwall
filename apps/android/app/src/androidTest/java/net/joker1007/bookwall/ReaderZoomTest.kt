...
/**
 * End-to-end coverage for pinch-zoom in the image reader: pinching magnifies the
 * page, a swipe while zoomed does not page (which would reset the zoom), and an
 * explicit page turn resets back to fit.
 */
...
class ReaderZoomTest {
    ...
    @Test
    fun pinchZoomsAndPageTurnResetsIt() {
        openReader()

        composeRule.onReaderScreen {
            // Pinching out magnifies the current page.
            pinchToZoom()
            assertZoomed()

            // While zoomed, a swipe pans instead of paging: the page never
            // settles on a new index, so the zoom is still in effect.
            swipePage()
            assertZoomed()

            // An explicit tap-zone page turn changes the page and resets to fit.
            tapLeftZone()
            assertNotZoomed()
        }
    }
    ...
    private companion object {
        const val SERVER_NAME = "Zoom E2E"
        const val BOOK_TITLE = "Zoomable Book"
        ...
        // The reader opens on page 1 (no pse:lastRead) so a forward tap turns a page.
        val FEED = """
