...
/**
 * The thumbnail overlay lists every page; picking one jumps there and closes
 * the overlay, while the close button and Back leave the page untouched.
 */
@RunWith(AndroidJUnit4::class)
class ReaderThumbnailGridTest {
    ...
    @Test
    fun pickingAThumbnailJumpsToThatPage() {
        openReader()
        composeRule.onReaderScreen {
            tapCenterZone()
            assertPageIndicator("1 / 12")

            openThumbnails()
            clickThumbnail(8)
            assertThumbnailsHidden()
            assertPageIndicator("9 / 12")
    ...
    private companion object {
        const val SERVER_NAME = "Thumbnail E2E"
        const val BOOK_TITLE = "Thumb Book"
